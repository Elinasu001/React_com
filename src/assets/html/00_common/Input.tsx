...
import { InfoOutlined, Search, Clear } from "@mui/icons-material";
...
export const SearchBox = ({
  ...
  <Box className={`form-input ${isTotal ? 'total' : ''}`}>
    {label && <FormLabel>{label}</FormLabel>}
    <Box>
      <Input
        className={`search ${isTotal ? 'total' : ''}`}
      ...
      <Box sx={{ position: 'absolute', right: 0, display: 'flex', gap: 0.5 }}>
        ...
        <IconButton className="search" onClick={onSearch}>
        </IconButton>
      </Box>
    </Box>
  </Box>
